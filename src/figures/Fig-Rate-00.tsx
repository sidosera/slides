...
import { Node, Arrow, Edge, Annotation, GroupLabel, Label } from "../diagram/grammar";
...
export function Figure03Rate() {
  ...
  const series = [
    { name: "series a", color: C.elasticBlue, vals: [95, 103, 4, 11], reset: 1 },
  ...
  return (
    ...
    >
      ...
      <GroupLabel x={40} y={top - 22} color={C.darkGray}>
        NAIVE — INSUFFICIENT
      </GroupLabel>
      {["scan", "group", "aggregate"].map((l, i) => {
        ...
        return (
          <g key={l}>
            ...
            {i < 2 && <Arrow x1={110} y1={y + h} x2={110} y2={y + step} variant="muted" />}
          </g>
        );
      })}
      ...
      {stages.map((s, i) => {
        ...
        return (
          <g key={s}>
            ...
            {i < stages.length - 1 && (
              <Arrow x1={mx + mw / 2} y1={y + h} x2={mx + mw / 2} y2={y + step} variant="logical" />
            )}
          </g>
        );
      })}
      ...
      {series.map((s, si) => {
        ...
        return (
          <g key={s.name}>
            <Label x={x0} y={baseY - 22} anchor="start" color={s.color} size={11} weight={600}>
              {s.name}
            </Label>
            {s.vals.map((v, i) => {
              const x = x0 + i * dx;
              ...
              return (
                <g key={i}>
                  ...
                  <circle cx={x} cy={baseY} r={4.5} fill={isResetEnd ? C.poppy : s.color} />
                  <Label x={x} y={baseY - 16} color={isResetEnd ? C.darkPoppy : C.ink} size={11} mono>
                    {String(v)}
                  </Label>
                </g>
              );
            ...
            {s.reset >= 0 && (
              <Annotation x={x0 + s.reset * dx - 4} y={baseY + 22} anchor="middle" color={C.darkPoppy}>
                counter reset
              </Annotation>
            )}
          </g>
      ...
      <Annotation x={640} y={top + 40 + 2 * 96 - 6} anchor="start" color={C.darkGray}>
